...
Page({
  ...
  onLoad: function(options) {
    ...
    const headerPaddingTop = statusBarHeightRpx + safeAreaTop + 20 // 20rpx基础padding
  ...
  addToCartAction: function(product, quantity, spec) {
    const app = getApp()
    app.addToCart(product, quantity, spec)
    this.hideModal()
  },
  ...
  filterProducts: function(keyword) {
    ...
    if (currentCategory > 0) {
      // 这里可以根据分类ID筛选，暂时显示全部
      // filteredProducts = products.filter(p => p.categoryId === currentCategory);
    }
